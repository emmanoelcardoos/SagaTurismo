...
// ── CONTEÚDO DA PÁGINA (DENTRO DO SUSPENSE) ──
function HotelDetalheContent({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    // Sincronização: Pega os dados da URL vindos da página de listagem
      ...
      <div className="relative w-full h-[40vh] md:h-[55vh] bg-slate-900 mt-[65px] md:mt-[80px]">
        ...
        <div className="absolute bottom-10 left-5 md:left-16 right-5 text-left">
          <Link href="/hoteis" className="inline-flex items-center gap-2 text-[10px] font-black uppercase text-white/70 mb-4 hover:text-white transition-colors">
            <ArrowLeft size={14}/> Voltar aos Hotéis
          </Link>
        ...
        <div className="flex-1 w-full space-y-8">
          ...
          <section className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden text-left">
             <div className="bg-slate-50 p-6 border-b border-slate-200 flex items-center justify-between">
               <div>
                  <h3 className={`${jakarta.className} text-xl md:text-2xl font-black text-slate-900`}>Acomodações</h3>
                  <p className="text-xs md:text-sm text-slate-500 font-medium">Preço total para {noites} {noites === 1 ? 'noite' : 'noites'}</p>
               </div>
               <div className="hidden sm:flex items-center gap-2 bg-[#009640]/10 text-[#009640] px-3 py-1.5 rounded-lg text-[10px] font-black uppercase">
                  <CheckCircle2 size={14}/> Preço Final
               </div>
             </div>
                ...
                <div className="border border-slate-200 rounded-3xl overflow-hidden bg-white shadow-sm flex flex-col">
                   ...
                   <div className="flex flex-col xl:flex-row">
                      <div className="w-full xl:w-1/3 p-5">
                         ...
                         <div className="grid grid-cols-2 gap-3 mt-4 text-[10px] md:text-xs text-slate-600 font-medium">
                            <span className="flex items-center gap-1.5"><Wind size={14}/> Ar-condic.</span>
                      ...
                      <div className="flex-1 p-6 flex flex-col sm:flex-row gap-6">
                         <div className="flex-1 space-y-4">
                            <p className="text-[10px] font-black uppercase text-[#00577C] tracking-widest">Incluso na Reserva</p>
                            <ul className="space-y-2">
                               <li className="flex items-center gap-2 text-xs font-bold text-[#009640]"><CheckCircle2 size={14}/> Cancelamento Gratuito</li>
                         ...
                         <div className="sm:w-56 bg-slate-50 rounded-2xl p-5 flex flex-col items-center sm:items-end justify-center border border-slate-100">
                            ...
                            <p className={`${jakarta.className} text-3xl font-black text-slate-900 leading-none mb-1`}>{formatarMoeda(precoTotal(hotel.quarto_standard_preco))}</p>
                            <p className="text-[10px] font-bold text-slate-400 mb-4">Já com impostos</p>
                            <button onClick={() => handleReserva('standard')} className="w-full bg-[#00577C] text-white py-3 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all">Reservar</button>
                         </div>
                      </div>
                   </div>
                </div>
                   ...
                   <div className="flex flex-col xl:flex-row">
                      <div className="w-full xl:w-1/3 p-5">
                         ...
                         <div className="grid grid-cols-2 gap-3 mt-4 text-[10px] md:text-xs text-slate-600 font-medium">
                            ...
                            <span className="flex items-center gap-1.5"><Coffee size={14}/> Café Manhã</span>
                         </div>
                      </div>
                      <div className="flex-1 p-6 flex flex-col sm:flex-row gap-6">
                         <div className="flex-1 space-y-4">
                            <p className="text-[10px] font-black uppercase text-[#00577C] tracking-widest">Upgrade Premium</p>
                            <ul className="space-y-2">
                               ...
                               <li className="flex items-center gap-2 text-xs font-bold text-[#009640]"><ShieldCheck size={14}/> Seguro Viagem Incluso</li>
                            </ul>
                         </div>
                         <div className="sm:w-56 bg-blue-50/50 rounded-2xl p-5 flex flex-col items-center sm:items-end justify-center border border-blue-100">
                            <p className="text-[10px] font-black text-[#00577C] uppercase mb-1">Valor Total</p>
                            <p className={`${jakarta.className} text-3xl font-black text-[#00577C] leading-none mb-1`}>{formatarMoeda(precoTotal(hotel.quarto_luxo_preco))}</p>
                            <p className="text-[10px] font-bold text-[#00577C]/60 mb-4">Melhor preço online</p>
                            <button onClick={() => handleReserva('luxo')} className="w-full bg-[#F9C400] text-[#00577C] py-3 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all">Reservar</button>
                         </div>
                      </div>
                   </div>
                </div>
