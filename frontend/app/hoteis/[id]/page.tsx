  ...
  return (
    <div className={`${inter.className} min-h-screen bg-[#F5F7FA] text-slate-900 flex flex-col`}>
      ...
      <header className={`fixed left-0 top-0 z-50 w-full border-b border-slate-200 bg-white/95 backdrop-blur-xl transition-transform duration-300 ${showHeader ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        {/* Menu Mobile */}
        {isMobileMenuOpen && (
          <div className="absolute top-[100%] left-0 w-full bg-white border-b border-slate-200 p-5 flex flex-col gap-4 shadow-xl lg:hidden animate-in slide-in-from-top-4 text-left">
      ...
      <div className="w-full h-[40vh] md:h-[50vh] relative bg-[#002f40] mt-[60px] md:mt-[90px]">
        <Link href="/hoteis" className="absolute top-4 md:top-6 left-4 md:left-6 z-20 flex items-center gap-2 text-xs md:text-sm font-bold text-slate-800 bg-white hover:bg-slate-50 px-3 md:px-4 py-1.5 md:py-2 rounded-full shadow-lg transition-colors">
          <ArrowLeft size={16} className="w-4 h-4 md:w-5 md:h-5" /> Voltar
        </Link>
        ...
        <div className="absolute bottom-6 md:bottom-10 left-5 md:left-16 z-20 text-left pr-5">
          <div className="flex items-center gap-2 mb-2 md:mb-3">
             <span className="bg-[#F9C400] text-[#00577C] px-2 md:px-3 py-1 md:py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-md">
               {hotel.tipo}
             </span>
      ...
      <div className="sticky top-[60px] md:top-[88px] z-40 bg-white border-b border-slate-200 shadow-sm lg:hidden">
         <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between gap-4 overflow-x-auto">
            ...
            <button 
               ...
               className="ml-auto bg-slate-50 p-2.5 rounded-full border border-slate-200 text-[#00577C] hover:bg-[#00577C] hover:text-white transition-all shrink-0 shadow-sm"
            >
               <Edit3 size={16}/>
            </button>
         </div>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 md:px-5 py-8 md:py-12 flex flex-col lg:flex-row items-start gap-8 relative z-10">
        ...
        <div className="flex-1 w-full min-w-0 flex flex-col gap-6 md:gap-8">
          ...
          <section className="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden text-left">
             <div className="bg-slate-50 p-5 md:p-6 border-b border-slate-200">
               ...
               <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Selecione o seu quarto para verificar os detalhes da reserva.</p>
             </div>
             
             <div className="p-4 md:p-6 flex flex-col gap-6 md:gap-8">
                
                {/* ── CARD: QUARTO STANDARD ── */}
                <div className="border border-slate-200 rounded-2xl md:rounded-[1.5rem] overflow-hidden bg-white shadow-sm flex flex-col hover:border-[#00577C]/30 transition-colors">
                   ...
                   <div className="flex flex-col xl:flex-row">
                      ...
                      <div className="w-full xl:w-2/5 p-4 md:p-5 border-b xl:border-b-0 xl:border-r border-slate-100 flex flex-col">
                         ...
                         <div className="grid grid-cols-2 gap-2 md:gap-3 text-[10px] md:text-xs text-slate-600 font-medium mt-auto">
                            <span className="flex items-center gap-1.5"><Wind size={12} className="md:w-[14px] md:h-[14px]"/> Ar-condicionado</span>
                            <span className="flex items-center gap-1.5"><Wifi size={12} className="md:w-[14px] md:h-[14px]"/> Wi-Fi Grátis</span>
                            <span className="flex items-center gap-1.5"><Bath size={12} className="md:w-[14px] md:h-[14px]"/> Banheiro Priv.</span>
                         </div>
                      </div>
                      ...
                      <div className="w-full xl:w-3/5 flex flex-col sm:flex-row">
                         <div className="flex-1 p-4 md:p-5 border-b sm:border-b-0 sm:border-r border-slate-100 flex flex-col justify-center">
                            ...
                            <ul className="space-y-2 md:space-y-3">
                               <li className="flex items-start gap-2 text-xs md:text-sm text-[#009640] font-bold">
                                 <CheckCircle2 size={16} className="shrink-0 mt-0.5" /> Cancelamento grátis até 24h
                               </li>
                               <li className="flex items-start gap-2 text-xs md:text-sm text-[#00577C] font-bold">
                                 <Zap size={16} className="shrink-0 mt-0.5" /> Confirmação Imediata
                               </li>
                               <li className="flex items-start gap-2 text-xs md:text-sm text-slate-600 font-medium">
                                 <CreditCard size={16} className="shrink-0 mt-0.5" /> Pagamento flexível
                               </li>
                            </ul>
                         </div>
                         <div className="w-full sm:w-48 p-5 flex flex-col items-center sm:items-end justify-center bg-slate-50/50">
                            ...
                            <p className={`${jakarta.className} text-2xl md:text-3xl font-black text-slate-900 leading-none mb-1`}>{formatarMoeda(totalStandard)}</p>
                            <p className="text-[9px] md:text-[10px] text-slate-500 font-bold mb-4 md:mb-5 text-center sm:text-right">{totalNoites} noite(s) · {quartos} quarto(s)</p>
                            <button onClick={() => handleReserva('standard')} className="w-full bg-[#00577C] hover:bg-[#004a6b] text-white py-3.5 md:py-3 rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest transition-colors shadow-md active:scale-95">
                               Reservar
                            </button>
                         </div>
                      </div>
                ...
                <div className="border-2 border-[#00577C]/20 rounded-2xl md:rounded-[1.5rem] overflow-hidden bg-white shadow-sm flex flex-col relative">
                   <div className="absolute top-0 right-0 bg-[#00577C] text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest px-3 md:px-4 py-1.5 rounded-bl-xl shadow-sm z-10 flex items-center gap-1.5">
                      <Award size={12} className="md:w-3.5 md:h-3.5"/> Mais Escolhido
                   </div>
                   ...
                   <div className="flex flex-col xl:flex-row">
                      <div className="w-full xl:w-2/5 p-4 md:p-5 border-b xl:border-b-0 xl:border-r border-slate-100 flex flex-col">
                         ...
                         <div className="grid grid-cols-2 gap-2 md:gap-3 text-[10px] md:text-xs text-slate-600 font-medium mt-auto">
                            <span className="flex items-center gap-1.5"><Wind size={12} className="md:w-[14px] md:h-[14px]"/> Ar-condicionado</span>
                            <span className="flex items-center gap-1.5"><Wifi size={12} className="md:w-[14px] md:h-[14px]"/> Wi-Fi Grátis</span>
                            <span className="flex items-center gap-1.5"><Bath size={12} className="md:w-[14px] md:h-[14px]"/> Banheira/Spa</span>
                         </div>
                      </div>
                      ...
                      <div className="w-full xl:w-3/5 flex flex-col sm:flex-row">
                         ...
                         <div className="w-full sm:w-48 p-5 flex flex-col items-center sm:items-end justify-center bg-slate-50/50">
                            ...
                            <p className={`${jakarta.className} text-2xl md:text-3xl font-black text-[#00577C] leading-none mb-1`}>{formatarMoeda(totalLuxo)}</p>
                            <p className="text-[9px] md:text-[10px] text-slate-500 font-bold mb-4 md:mb-5 text-center sm:text-right">{totalNoites} noite(s) · {quartos} quarto(s)</p>
                            <button onClick={() => handleReserva('luxo')} className="w-full bg-[#F9C400] hover:bg-[#e5b500] text-[#00577C] py-3.5 md:py-3 rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest transition-colors shadow-md active:scale-95">
                               Reservar
                            </button>
                         </div>
                      </div>
             ...
             <div className="flex flex-col lg:flex-row gap-8 md:gap-10">
                ...
                <div className="w-full lg:w-1/3">
                   <div className="flex items-center gap-4 mb-5 md:mb-6">
                      <div className="bg-[#00577C] text-white text-2xl md:text-3xl font-black rounded-tl-xl rounded-tr-xl rounded-br-xl rounded-bl-sm w-14 h-12 md:w-16 md:h-14 flex items-center justify-center shadow-lg">
                      ...
                      <div>
                         ...
                         <p className="text-[10px] md:text-xs text-slate-500 font-bold">{hotel.avaliacoes_info?.total || 73} avaliações verificadas</p>
                      </div>
                   </div>
                   ...
                   <div className="space-y-3 md:space-y-4">
                      {[
                        ...
                        { label: 'Atendimento', score: hotel.avaliacoes_info?.atendimento || 8.5 }
                         ...
                         <div key={item.label}>
                            <div className="flex justify-between text-[10px] md:text-xs font-bold text-slate-700 mb-1">
                               <span>{item.label}</span>
                               <span className="text-[#00577C]">{item.score.toFixed(1)}</span>
                            </div>
                            <div className="w-full bg-slate-200 rounded-full h-1 md:h-1.5">
                               <div className="bg-[#00577C] h-1 md:h-1.5 rounded-full" style={{ width: `${(item.score / 10) * 100}%` }}></div>
                            </div>
                         </div>
                      ))}
                   </div>
                ...
                <div className="w-full lg:w-2/3">
                   <div className="flex flex-wrap gap-2 mb-5 md:mb-6">
                      <span className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest cursor-pointer hover:bg-slate-200">Todas as avaliações</span>
                      <span className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest cursor-pointer hover:bg-slate-200">Estadia excelente!</span>
                      <span className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest cursor-pointer hover:bg-slate-200">Localização ótima</span>
                   </div>
                   ...
                   <div className="space-y-5 md:space-y-6">
                      ...
                      <div className="border-b border-slate-100 pb-5 md:pb-6">
                         <div className="flex items-center gap-3 mb-2 md:mb-3">
                            <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-[#00577C] font-black text-[10px] md:text-xs">M</div>
                            <div>
                               <p className="text-xs md:text-sm font-bold text-slate-800 leading-none">Maria Silva</p>
                               <p className="text-[9px] md:text-[10px] text-slate-400 mt-1">Hospedou-se em Maio 2026</p>
                            </div>
                         </div>
                         <p className="text-xs md:text-sm text-slate-600 font-medium leading-relaxed italic">"O hotel superou as expetativas. Quartos muito limpos, vista maravilhosa para a natureza e os funcionários foram extremamente prestáveis durante todo o fim de semana."</p>
                      </div>
                      {/* Review Mock 2 */}
                      <div>
                         <div className="flex items-center gap-3 mb-2 md:mb-3">
                            <div className="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center text-[#d9a000] font-black text-[10px] md:text-xs">C</div>
                            <div>
                               <p className="text-xs md:text-sm font-bold text-slate-800 leading-none">Carlos F.</p>
                               <p className="text-[9px] md:text-[10px] text-slate-400 mt-1">Hospedou-se em Abril 2026</p>
                            </div>
                         </div>
                         <p className="text-xs md:text-sm text-slate-600 font-medium leading-relaxed italic">"Excelente relação qualidade-preço. O pequeno-almoço tinha muitas opções locais. Apenas a internet oscilou um pouco à noite, mas no geral foi fantástico."</p>
                      </div>
                   </div>
                   
                   <button className="mt-5 md:mt-6 text-[#00577C] font-black text-xs md:text-sm hover:underline">Mostrar todas as avaliações</button>
                </div>
             </div>
          </section>
          ...
          <section className="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-sm p-5 md:p-10 text-left">
             <h3 className={`${jakarta.className} text-xl md:text-2xl font-black text-slate-900 mb-6 md:mb-8`}>Políticas da Propriedade</h3>
             <div className="grid md:grid-cols-[1fr_2fr] gap-x-8 gap-y-5 md:gap-y-6 text-xs md:text-sm">
                
                <div className="font-black text-slate-800">Horários de check-in e check-out</div>
                <div className="text-slate-600 font-medium space-y-1.5 md:space-y-2">
                   ...
                   <p className="text-[10px] md:text-xs text-slate-500 mt-1.5 md:mt-2">Os horários podem variar conforme a ocupação. Contacte a propriedade para mais flexibilidade.</p>
                </div>

                <div className="col-span-1 md:col-span-2 h-px bg-slate-100 my-1 md:my-2"></div>
          ...
          <section className="bg-[#002f40] rounded-[1.5rem] md:rounded-[2rem] border border-[#00577C] shadow-xl p-6 md:p-10 text-white relative overflow-hidden text-left">
             <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none"><Globe size={100} className="md:w-[150px] md:h-[150px]"/></div>
             <h3 className={`${jakarta.className} text-xl md:text-2xl font-black mb-2 relative z-10`}>Informações de Contacto</h3>
             <p className="text-white/70 text-xs md:text-sm font-medium mb-6 md:mb-8 relative z-10">Precisa de esclarecer alguma dúvida diretamente com a propriedade? Use os contactos oficiais abaixo.</p>
             
             <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 relative z-10">
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 md:p-5 border border-white/10 flex items-center sm:flex-col sm:items-start gap-4 sm:gap-0">
                   <Phone className="text-[#F9C400] mb-0 sm:mb-3 shrink-0" size={20} />
                   <div className="overflow-hidden">
                     <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white/50 mb-1">Telefone / WhatsApp</p>
                     <p className="font-bold text-xs md:text-sm truncate">{hotel.contatos?.telefone || '+55 (94) 90000-0000'}</p>
                   </div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 md:p-5 border border-white/10 flex items-center sm:flex-col sm:items-start gap-4 sm:gap-0">
                   <Mail className="text-[#F9C400] mb-0 sm:mb-3 shrink-0" size={20} />
                   <div className="overflow-hidden">
                     <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white/50 mb-1">E-mail</p>
                     <p className="font-bold text-xs md:text-sm truncate">{hotel.contatos?.email || 'contato@hotel.com.br'}</p>
                   </div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 md:p-5 border border-white/10 flex items-center sm:flex-col sm:items-start gap-4 sm:gap-0">
                   <Globe className="text-[#F9C400] mb-0 sm:mb-3 shrink-0" size={20} />
                   <div className="overflow-hidden">
                     <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white/50 mb-1">Website</p>
                     <p className="font-bold text-xs md:text-sm truncate">{hotel.contatos?.website || 'www.hotel-saga.com'}</p>
                   </div>
                </div>
             </div>
          </section>
        ...
        <div id="motor-reservas" className="w-full lg:w-[380px] shrink-0 lg:self-start lg:sticky lg:top-32 relative z-30">
          <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-slate-200 text-left">
             <h3 className={`${jakarta.className} text-lg md:text-xl font-black text-slate-900 mb-5 md:mb-6 flex items-center gap-2 md:gap-3`}>
             ...
             <div className="space-y-3 md:space-y-4 mb-5 md:mb-6">
                <div>
                  <label className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-[#00577C] block mb-2">Check-in — Check-out</label>
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 shadow-inner">
                     ...
                     <div className="flex items-center justify-between mb-3 md:mb-4">
                        <button onClick={() => setMesAtualCalendario(new Date(anoCorrente, mesCorrente - 1))} className="p-1 hover:bg-slate-200 rounded-full transition-colors"><ChevronLeft size={16} className="md:w-[18px] md:h-[18px]"/></button>
                        ...
                        <button onClick={() => setMesAtualCalendario(new Date(anoCorrente, mesCorrente + 1))} className="p-1 hover:bg-slate-200 rounded-full transition-colors"><ChevronRight size={16} className="md:w-[18px] md:h-[18px]"/></button>
                     </div>
             ...
             <div className="space-y-3 md:space-y-4 mb-5 md:mb-6 pt-4 border-t border-slate-100">
                ...
                <div className="flex items-center justify-between">
                   <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-[#00577C]">Crianças <span className="text-[7px] md:text-[8px] text-slate-400 block -mt-1 md:-mt-0.5">(Até 12 anos)</span></span>
                   <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg p-1">
                      ...
                      <span className="font-bold text-xs w-4 text-center">{criancas}</span>
             ...
             {!checkin || !checkout ? (
               <div className="mt-5 md:mt-6 p-4 bg-blue-50 text-[#00577C] rounded-2xl flex items-start gap-3 border border-blue-100">
                  <Info size={16} className="shrink-0 mt-0.5 md:w-[18px] md:h-[18px]" />
                  <p className="text-[10px] md:text-xs font-bold leading-relaxed">Selecione o check-in e check-out para calcular o valor final da estadia.</p>
               </div>
             ) : (
               <div className="mt-5 md:mt-6 p-4 bg-green-50 text-[#009640] rounded-2xl flex items-start gap-3 border border-green-100">
                  <CheckCircle2 size={16} className="shrink-0 mt-0.5 md:w-[18px] md:h-[18px]" />
                  <div>
                    <p className="text-[10px] md:text-xs font-bold leading-tight">Datas selecionadas!</p>
                    <p className="text-[9px] md:text-[10px] font-medium opacity-80 mt-1">Os preços já foram atualizados com base em {totalNoites} noites.</p>
                  </div>
               </div>
             )}
          </div>
      ...
      <footer className="border-t border-slate-200 bg-white mt-auto text-left">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 md:gap-8 px-5 py-8 md:py-12 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="relative h-10 w-28 md:h-14 md:w-40 shrink-0"><Image src="/logop.png" alt="Prefeitura" fill className="object-contain object-left md:object-center" /></div>
            <div className="hidden border-l border-slate-200 pl-4 md:block">
              ...
              <p className="text-xs text-slate-500 uppercase font-bold tracking-widest text-[9px] md:text-[10px]">Portal Oficial de Turismo</p>
            </div>
          </div>
          <p className="text-[9px] md:text-[10px] text-slate-400 font-medium uppercase tracking-widest text-center md:text-left">© {new Date().getFullYear()} · Prefeitura Municipal de São Geraldo do Araguaia</p>
        </div>
      </footer>
    </div>
  );
