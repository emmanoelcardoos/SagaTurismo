    ...
    <Reveal anim="up" delay={index * 100}>
      <article className={`bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all p-4 flex flex-col ${isPar ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 lg:gap-12 overflow-hidden group mb-10`}>
        ...
        <div className="relative w-full h-[350px] lg:h-auto lg:min-h-[450px] lg:w-[50%] rounded-3xl overflow-hidden bg-slate-100 shrink-0">
          ...
          {/* Etiqueta Dinâmica (Tipo da atração) */}
          {atracao.tipo && (
            <div className="absolute top-6 left-6 z-10 flex items-center gap-2 bg-white/90 backdrop-blur px-4 py-2 rounded-xl shadow-md text-[#00577C]">
              <Camera size={14} />
              <span className="text-[10px] font-black uppercase tracking-widest">{atracao.tipo}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 py-4 lg:py-12 px-2 lg:px-6 flex flex-col justify-center">
          ...
          <p className="text-slate-500 text-base md:text-lg leading-relaxed mb-8 font-medium">
            {atracao.descricao || 'Descubra a beleza única e os cenários inesquecíveis que esta atração oferece em São Geraldo do Araguaia.'}
          </p>
          ...
          <div className="flex flex-wrap gap-3 mb-10">
            {atracao.link_google_maps && (
              <a href={atracao.link_google_maps} target="_blank" rel="noopener noreferrer" 
                 className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-slate-50 border border-slate-200 text-slate-600 hover:text-[#00577C] hover:border-[#00577C]/30 transition-colors">
            ...
            {atracao.preco_entrada !== undefined && atracao.preco_entrada !== null && (
              <span className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-slate-50 border border-slate-200 text-slate-600">
                <Ticket size={14} className="text-[#009640]" />
...
export default function AtracoesPage() {
  ...
  return (
    <main className={`${inter.className} text-slate-900 overflow-x-hidden min-h-screen bg-[#FDFCF7]`}>
      ...
      {/* ══════════════════════════════════════
          HERO CLEAN (SEM DESFOQUE E SEM DESCRIÇÃO)
      ══════════════════════════════════════ */}
      <section className="relative h-[55vh] md:h-[65vh] flex flex-col justify-center items-center text-center px-6 overflow-hidden bg-[#002f40]">
        ...
        {/* Camada de escurecimento simples para leitura, sem o efeito embaçado */}
        ...
        <div className="relative z-10 max-w-3xl mx-auto pt-16">
          <Reveal anim="up">
            <h1 className={`${jakarta.className} text-5xl md:text-7xl font-black text-white leading-[1.1] drop-shadow-lg`}>
              Atrações <span className="italic text-[#F9C400]">Locais</span>
            </h1>
          </Reveal>
        </div>
      </section>
      ...
      ══════════════════════════════════════ */}
      <section id="atracoes" className="pb-24 px-6 md:px-12">
                ...
                <div className="rounded-[2.5rem] p-10 md:p-14 h-full flex flex-col justify-between min-h-[300px]"
                  ...
                  <div>
                    <h3 className={`${jakarta.className} text-4xl md:text-5xl font-black text-white leading-[1.1] mb-6`}>
                      Procurando uma viagem <br />
                      <span className="italic text-[#F9C400]">já organizada?</span>
                    </h3>
                    <p className="text-white/80 text-base leading-relaxed max-w-md">
                      Conheça nossas agências parceiras locais que oferecem roteiros seguros e imersivos pela região.
                    </p>
                  </div>
                  <Link href="/agencias"
                ...
                <div className="rounded-[2.5rem] p-10 h-full flex flex-col justify-between min-h-[300px] bg-[#F9C400]">
                  <div>
                    ...
                    <p className="text-[#002f40]/70 text-sm leading-relaxed font-medium">
                      Tem 50% de desconto na entrada do parque Cachoeira Três Quedas. Faça já o seu cadastro.
                    </p>
                  </div>
                  <Link href="/cadastro"
