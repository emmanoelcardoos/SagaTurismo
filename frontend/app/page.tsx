...
export default function HomePage() {
  ...
  return (
    <main className={`${inter.className} bg-white text-slate-900 overflow-x-hidden`}>
      ...
      {/* ── HEADER EDITORIAL ── */}
      <header className={`fixed left-0 top-0 z-50 w-full transition-all duration-300 ${showHeader ? 'translate-y-0' : '-translate-y-full'} ${isScrolled ? 'bg-white/95 backdrop-blur-xl py-4 shadow-sm border-b border-slate-100' : 'bg-transparent py-6'}`}>
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-4">
            <div className="relative h-12 w-40">
              <Image src="/logop.png" alt="SagaTurismo" fill className={`object-contain transition-all ${!isScrolled ? 'brightness-[100] invert' : ''}`} />
            </div>
            {isScrolled && (
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {['Atracoes','Hoteis', 'Pacotes', 'Passeios', 'Aldeias', 'Gastronomia', 'Biodiversidade', 'Comunidades'].map(item => (
              <Link key={item} href={`/${item.toLowerCase()}`}
                className={`text-[11px] font-black uppercase tracking-[0.2em] transition-colors ${isScrolled ? 'text-slate-600 hover:text-[#00577C]' : 'text-white/80 hover:text-white'}`}>
                {item}
              </Link>
            ))}
            <div className="flex items-center gap-3 border-l border-white/20 pl-6 ml-2">
              <Link href="/parceiros" className={`text-[11px] font-black uppercase tracking-[0.2em] transition-colors ${isScrolled ? 'text-slate-600 hover:text-[#00577C]' : 'text-white/80 hover:text-white'}`}>
                Parceiros
              </Link>
              <Link href="/cadastro" className="bg-[#F9C400] text-[#00577C] px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform shadow-lg">
                Residentes
              </Link>
            </div>
          </nav>

          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`lg:hidden p-2 ${isScrolled ? 'text-[#00577C]' : 'text-white'}`}>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="absolute top-[100%] left-0 w-full bg-white border-b border-slate-200 p-5 flex flex-col gap-4 shadow-xl lg:hidden">
            {['Hoteis', 'Pacotes', 'Passeios', 'Aldeias', 'Gastronomia'].map(item => (
              <Link key={item} href={`/${item.toLowerCase()}`} onClick={() => setIsMobileMenuOpen(false)} className="font-bold text-slate-700 text-lg">{item}</Link>
            ))}
            <Link href="/parceiros" onClick={() => setIsMobileMenuOpen(false)} className="font-bold text-slate-700 text-lg">Parceiros</Link>
