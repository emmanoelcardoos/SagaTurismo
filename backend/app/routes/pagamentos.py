...
def calcular_preco_hotel_dinamico(hotel_id: str, tipo_quarto: str, checkin_str: str, checkout_str: str) -> float:
    ...
    # Sincronizado para ordenar pela regra mais recente e ignorar as duplicadas antigas
        ...
        for regra in excecoes_calendario:
            ...
            if regra_inicio <= d_atual <= regra_fim:
                if not regra.get("disponivel", True):
                    raise HTTPException(
                        status_code=400, 
                        detail=f"Acomodação esgotada para a data de {d_atual.strftime('%d/%m/%Y')}."
                    )
                preco_da_noite = float(regra["preco"])
                break
        
        subtotal_hospedagem += preco_da_noite if preco_da_noite is not None else base_preco
...
async def processar_pagamento(pedido: PedidoPagamento):
    try:
        ...
        pedido_db = {
            "codigo_pedido": codigo_pedido, "tipo_item": pedido.tipo_item, "nome_cliente": pedido.nome_cliente,
            "cpf_cliente": pedido.cpf_cliente, "email_cliente": pedido.email_cliente, "telefone_cliente": pedido.telefone_cliente,
            "valor_total": valor_total, "status_pagamento": "aguardando", "metodo_pagamento": pedido.metodo_pagamento,
            "data_checkin": pedido.data_checkin, "data_checkout": pedido.data_checkout, "data_nascimento": pedido.data_nascimento,
            "foto_url": pedido.foto_url, "quantidade": pedido.quantidade
        }
        if item_id_db:
        ...
        payload_pagbank = {
            ...
            "customer": {
                "name": pedido.nome_cliente, "email": pedido.email_cliente, "tax_id": tax_id_limpo,
        ...
        if pedido.metodo_pagamento == "pix":
            ...
            charge = {
                "reference_id": codigo_pedido, "description": nome_item_checkout,
                "amount": {"value": int(valor_total * 100), "currency": "BRL"},
                "payment_method": {
                    "type": "CREDIT_CARD", "installments": pedido.parcelas, "capture": True,
                    "card": {"encrypted": pedido.encrypted_card},
                    "holder": { "name": pedido.nome_cliente, "tax_id": tax_id_limpo, "address": pedido.endereco_faturacao.dict() }
                }
            }
            if splits_array: 
